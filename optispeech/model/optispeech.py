...
PAUSE_SYMBOLS = (" ", "_+_", "_sil_", ".", ",", "?", "!", ":", ";")
DEFAULT_PAUSE_FACTOR = 2.0
...
class OptiSpeech(BaseLightningModule):
    def __init__(
        ...
        input_symbols = self.text_processor.tokenizer.input_symbols
        pause_token_ids = tuple(input_symbols[symbol] for symbol in PAUSE_SYMBOLS if symbol in input_symbols)
        self.pause_token_ids = pause_token_ids
    ...
    def synthesise(self, inputs: InferenceInputs) -> InferenceOutputs:
        ...
        synth_outputs = self.generator.synthesise(
            ...
            pause_factor=inputs.pause_factor,
            pause_mask=inputs.pause_mask,
        )
    ...
    def prepare_input(
        ...
        *,
        ...
        Args:
            ...
            pause_factor (float|None): duration multiplier for spaces, silence tokens, and punctuation
        ...
        if pause_factor is None:
            pause_factor = getattr(self.inference_args, "pause_factor", DEFAULT_PAUSE_FACTOR)
        if pause_factor < 1.0:
            raise ValueError("pause_factor must be greater than or equal to 1.0")

        sequences = input_ids if split_sentences else [input_ids]
        pause_mask = [[token_id in self.pause_token_ids for token_id in sequence] for sequence in sequences]
        get_separator_mask = getattr(self.text_processor.tokenizer, "get_pause_separator_stretch_mask", None)
        separator_token_id = self.text_processor.tokenizer.input_symbols.get("_+_")
        if get_separator_mask is not None and separator_token_id is not None and len(sequences) == 1:
            separator_positions = [
                index for index, token_id in enumerate(sequences[0]) if token_id == separator_token_id
            ]
            separator_mask = get_separator_mask(text)
            if len(separator_mask) == len(separator_positions):
                for position, should_stretch in zip(separator_positions, separator_mask):
                    pause_mask[0][position] = should_stretch
