...
class TextWavDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.file_paths = []
        for file_pth in parse_filelist(filelist_path):
            item = self.get_datapoint(file_pth)
            if item["x"].numel() == 0:
                log.warning("Skipping sample with empty phoneme_ids: %s", file_pth)
                continue
            if item["mel"].shape[-1] <=  1800:
                self.file_paths.append(file_pth)
        random.seed(seed)
...
class TextWavBatchCollate:
    ...
    def __call__(self, batch):
        ...
        empty_items = [item["filepath"] for item in batch if item["x"].numel() == 0]
        if empty_items:
            raise ValueError(f"Batch contains samples with empty phoneme_ids: {empty_items}")
